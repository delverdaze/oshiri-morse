/* おしりモールス UI（DOM・音声・クリップボード・kuromojiロード）
   変換コアは morse.js に分離。本ファイルは classic script として
   morse.js の後に読み込むこと（encode/decode/各定数/tokenizer 等の
   グローバルを参照するため）。 */
/* ===== UI ===== */
let mode = "enc";
function setMode(m){
...
function runEncode(){
  ...
  if(!v.trim()){
    ...
    document.getElementById("enc-yomi").value="";
    updateXPostCounter("");
    note.textContent="";
    return;
  }
  if(KANJI_RE.test(v) && !tokenizer && dictState !== "failed"){
    requestDict();
    document.getElementById("enc-out").value="";
    document.getElementById("enc-yomi").value="";
    updateXPostCounter("");
...
function updateXPostCounter(text){
  const status = document.getElementById("enc-x-status");
  const preview = document.getElementById("enc-x-preview");
  const previewText = document.getElementById("enc-x-preview-text");
...
function copyOut(id, noteId){
  ...
  if(!txt){ return; }
  const done = (ok)=>{
    if(ok){ note.className="note ok"; note.textContent="📋 コピーしました！"; puffFrom(el, 3); }
...
/* ===== 🔊 お尻ペチペチ音エンジン（Web Audio・音源ファイル不要） ===== */
let audioCtx = null;
...
// 実際の出力アンロックは、再生ボタン側（非passiveのonclick）の unlockKick が担う。
